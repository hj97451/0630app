...
const Settings = (() => {
    ...
    return `
      <div class="modal-box">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" onclick="Settings.save()">코드 변경</button>
        </div>
      </div>
    `;
  }
  ...
  function save() {
    ...
    // 입력 검증
    if (!current || !newCode || !confirm) {
      showError(errEl, "모든 칸을 입력해주세요.");
      return;
    }

    // 현재 코드 확인
    const auth = Storage.authenticateStudent(
      _currentStudent.number,
      _currentStudent.name,
      current
    );
    if (!auth) {
      showError(errEl, "현재 코드가 맞지 않아요.");
      return;
    }

    // 새 코드 길이 검사
    if (newCode.length < 4) {
      showError(errEl, "새 코드는 4자 이상으로 만들어주세요.");
      return;
    }

    // 일치 확인
    if (newCode !== confirm) {
      showError(errEl, "새 코드가 서로 달라요. 다시 확인해주세요.");
      return;
    }

    // 저장
    const ok = Storage.updateStudentCode(_currentStudent.number, newCode);
    if (ok) {
      close();

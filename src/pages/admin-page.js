...
const AdminPage = (() => {
    ...
    document.getElementById("admin-app").innerHTML = `
      <div class="admin-login-wrapper">
        <div class="admin-login-card">
          <h1>🏫 교사 관리 페이지</h1>
          <p>${APP_CONFIG.class.school} ${APP_CONFIG.class.name} · ${APP_CONFIG.class.year}년도</p>
          ...
          <div id="admin-login-error" class="form-error" style="display:none; margin-bottom:12px;"></div>
  ...
  async function login() {
    ...
    const btn   = document.querySelector(".admin-login-card .btn-primary");
    if (btn) { btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> 확인 중...'; }

    const ok = await Storage.verifyAdminPassword(pw);
  ...
  function logout() { _loggedIn = false; _renderLogin(); }
  ...
  async function _renderDashboard() {
    ...
    const submissions   = _selectedAssignId
      ? await Storage.getSubmissionsByAssignment(_selectedAssignId)
      : [];
    ...
    const assignTabs = allAssignments.map(a => `
      <button class="assignment-tab ${a.id === _selectedAssignId ? "active" : ""}"
    ...
    const studentRows = allStudents.map(s => {
      ...
      const typeStr = sub ? (typeMap[sub.content?.type] || "") : "";
        ...
        <tr>
          ...
          <td data-label="상태">
            <span class="submit-status ${isDone ? "done" : "pending"}">
              <span class="status-dot"></span>${isDone ? "제출 완료" : "미제출"}
            </span>
          </td>
          <td data-label="형식">${typeStr}</td>
          <td class="td-time"   data-label="제출 시각">${timeStr}</td>
          <td class="td-action" data-label="내용">
            <button class="view-btn" onclick="AdminPage.viewSubmission(${s.number})" ${isDone ? "" : "disabled"}>보기</button>
          </td>
        </tr>
      `;
    }).join("");
    ...
    document.getElementById("admin-app").innerHTML = `
      <div class="admin-layout">
        <header class="admin-header">
          <div>
            <h1>${APP_CONFIG.class.school} ${APP_CONFIG.class.name} 과제 현황</h1>
        ...
        <div class="stats-grid">
          <div class="stat-card"><div class="stat-value">${totalStudents}</div><div class="stat-label">전체 학생</div></div>
          <div class="stat-card"><div class="stat-value" style="color:var(--color-success);">${submittedCount}</div><div class="stat-label">제출 완료</div></div>
          <div class="stat-card"><div class="stat-value" style="color:var(--color-warning);">${pendingCount}</div><div class="stat-label">미제출</div></div>
          <div class="stat-card"><div class="stat-value">${rate}%</div><div class="stat-label">제출률</div></div>
        </div>
        ...
        <div class="student-table-wrapper">
          <div class="table-toolbar">
            ...
            <input class="table-search" type="text" placeholder="이름으로 검색..."
          ...
          <table class="student-table" id="student-table">
            <thead>
              <tr>
                <th>번호</th><th>이름</th><th>제출 여부</th><th>형식</th><th>제출 시각</th><th>내용 보기</th>
              </tr>
            </thead>
            <tbody id="student-tbody">${studentRows}</tbody>
          </table>
        </div>
      </div>
  ...
  async function viewSubmission(studentNumber) {
    const submission = await Storage.getSubmission(studentNumber, _selectedAssignId);
    if (!submission) return;

    const allStudents = await Storage.getAllStudents();
    const student     = allStudents.find(s => s.number === studentNumber);
    ...
    if (content.type === "text") {
      ...
      contentHTML = `<div class="submission-content-box" style="padding:0;overflow:hidden;">
        <img class="submission-img-preview" src="${content.data}" alt="제출 이미지" /></div>`;
    } else if (content.type === "pdf") {
      contentHTML = `<div class="submission-content-box" style="text-align:center;">
        <p>📄 ${_escapeHTML(content.filename || "PDF 파일")}</p>
        <a href="${content.data}" download="${content.filename || 'submission.pdf'}"
           class="btn btn-primary" style="margin-top:12px;display:inline-flex;">PDF 다운로드</a>
      </div>`;
    }
    ...
    overlay.innerHTML = `
      <div class="modal-box" style="max-width:560px;">
        ...
        <div class="modal-footer" style="margin-top:var(--space-lg);">
          ...
          <button class="btn btn-danger" onclick="AdminPage.adminDeleteSubmission(${studentNumber})">제출 삭제</button>
        </div>
      </div>
    `;
    overlay.classList.add("open");
    overlay.addEventListener("click", e => { if (e.target === overlay) AdminPage.closeSubmissionModal(); });
  }
  ...
  async function adminDeleteSubmission(studentNumber) {
    if (!window.confirm("이 학생의 제출을 삭제할까요?")) return;
    await Storage.deleteSubmission(studentNumber, _selectedAssignId);
  ...
  async function exportCSV() {
    ...
    const assignment  = APP_CONFIG.assignments.find(a => a.id === _selectedAssignId);
    const allStudents = await Storage.getAllStudents();
    const submissions = await Storage.getSubmissionsByAssignment(_selectedAssignId);
    ...
    const rows = [
      ...
      ...allStudents.map(s => {
        ...
        return [s.number, s.name, sub ? "O" : "X", sub ? Storage.formatDate(sub.submittedAt) : "", sub ? sub.content?.type : ""];
      }),
    ];
